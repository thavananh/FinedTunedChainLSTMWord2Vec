...
class CustomHyperModel(kt.HyperModel):
    ...
    def fit(self, hp, model, *args, **kwargs):
        ...
        with self.strategy.scope():
            ...
            # Convert the training data into a tf.data.Dataset
            # train_dataset = tf.data.Dataset.from_tensor_slices((self.X_train, self.y_train))
            # train_dataset = train_dataset.shuffle(buffer_size=10000)  # Shuffle the dataset
            # train_dataset = train_dataset.batch(hp.get('batch_size'))  # Batch the dataset
            # train_dataset = self.strategy.experimental_distribute_dataset(train_dataset)  # Distribute the dataset

            # # Similarly for validation and test datasets
            # val_dataset = tf.data.Dataset.from_tensor_slices((self.X_val, self.y_val))
            # val_dataset = val_dataset.batch(hp.get('batch_size'))
            ...
            # test_dataset = test_dataset.batch(hp.get('batch_size'))
            ...
            # Tạo báo cáo classification với F1-score
            report_1 = classification_report(y_true_labels_test, y_pred_labels_test, target_names=['Negative', 'Neutral', 'Positive'], zero_division=0, digits=3)
